...
function loadRowListContent(attempt = 1) {
   disableFilterButton();

   const currentUrl = new URL(window.location);

   let search_term = $(this).data('search') || $('#id_search').val();
  ...
  getDynamicJson(url, function(rows) {
    let finished_text = getRowsList(rows);
    let pagination_text = getPaginationText(rows, 200);
